...
export function ParentDashboard({ onClose }: { onClose: () => void }) {
  ...
  const stats = useMemo(() => {
    ...
    const totalCompletions = Object.values(activityProgress).filter(a => a.completed).length;

    const unlockedAchievements = achievements.filter(a => a.unlocked).length;
    const totalAchievementsCount = achievements.length;

    // Calculate average stars per activity
    const totalStarsEarned = Object.values(activityProgress).reduce(
      (sum, a) => sum + (a.starsEarned || 0), 0
    );
    ...
    Object.entries(activityProgress).forEach(([activityId, progress]) => {
      ...
      if (progress.completed) {
        moduleProgress[module].completed += 1;
      }
      moduleProgress[module].stars += progress.starsEarned || 0;
    });
  ...
  }, [totalStars, activityProgress, achievements]);
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-8">
          ...
          {activeTab === 'progress' && <ProgressTab stats={stats} activityProgress={activityProgress} />}
      ...
      <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {achievements
            ...
            .slice(0, 6)
            .map((achievement: any) => (
              <div key={achievement.id} className="bg-white rounded-xl p-4 shadow-lg">
...
// Progress Tab
function ProgressTab({ activityProgress }: any) {
  return (
    <div className="space-y-8">
      <div className="bg-gradient-to-r from-blue-100 to-cyan-100 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-2 gap-6">
          {Object.entries(activityProgress).map(([activityId, progress]: any) => (
            <div key={activityId} className="bg-white rounded-xl p-4 shadow-lg">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500">
                    {progress.completed ? 'Completed' : 'In Progress'} - {progress.attempts} attempts
                  </div>
                </div>
              </div>

              <div className="flex gap-1">
                {Array(progress.starsEarned || 0).fill(0).map((_: any, i: number) => (
                  <span key={i}>⭐</span>
                ))}
              </div>
            </div>
        ...
        <div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-lg">
            ...
            <div className="mt-4 space-y-2">
              {getBottomModules(stats.moduleProgress).map(([module]: any) => (
